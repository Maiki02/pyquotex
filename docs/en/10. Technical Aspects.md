...
#### Instance-scoped Session State (Multi-session)
- Mutable websocket/auth state is now instance-scoped (`self.state`).
- Each `Quotex` client keeps its own `ssid`, connection flags, error flags, and websocket write mutex state.
- This prevents cross-session data contamination when running multiple clients in the same process.

Base example for two sessions and two symbols:
```python
import asyncio
import time
