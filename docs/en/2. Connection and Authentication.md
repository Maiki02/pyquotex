...
### Automatic fallback for expired session tokens

Since April 2026, `connect()` includes a built-in fallback for stale sessions:

1. If `session.json` has a token, WebSocket connection is attempted with that token.
2. If authorization is rejected, in-memory session data is cleared.
3. HTTP authentication is executed again with `email/password`.
4. WebSocket connection is retried with the refreshed token.

This avoids hard failures caused by expired tokens and improves long-running process stability.
