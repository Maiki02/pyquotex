...
#### Estado de sesión por instancia (multi-sesión)
- El estado mutable de conexión/WebSocket ahora es por instancia de cliente (`self.state`).
- Cada objeto `Quotex` mantiene su propio `ssid`, flags de conexión, estado de error y exclusión de escritura.
- Esto evita contaminación cruzada cuando se ejecutan varias sesiones en paralelo dentro del mismo proceso.

Ejemplo base para dos sesiones y dos símbolos:
```python
import asyncio
import time
...
Nota: para consumo concurrente robusto, se recomienda 1 cliente por instrumento/sesión.
