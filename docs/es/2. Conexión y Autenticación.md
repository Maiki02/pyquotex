...
### Fallback automático cuando el token está vencido

Desde abril de 2026, `connect()` incorpora un fallback automático para sesiones vencidas:

1. Si `session.json` tiene token, se intenta conexión WebSocket con ese token.
2. Si el servidor rechaza la autorización, la librería limpia la sesión en memoria.
3. Se ejecuta autenticación HTTP con `email/password`.
4. Se reintenta la conexión WebSocket con el token nuevo.

Esto evita que el cliente falle de forma definitiva por un token expirado y reduce caídas en procesos de larga ejecución.
