...
### Comportamiento interno de get_candles (actualizado)

- `get_candles()` usa una suscripción mínima (`subscribe_realtime_candle`) para forzar la entrega del histórico cuando el broker lo requiere.
- En modo snapshot (`progressive=False`) la suscripción se cierra siempre en `finally` con `unsubscribe_realtime_candle`.
- En modo progresivo (`progressive=True`) mantiene canales adicionales para flujo en tiempo real.

Este diseño evita timeouts por falta de trigger y, al mismo tiempo, previene fugas de red en consultas históricas puntuales.

### Flujo Confiable de Velas (Recomendado)
```python
import asyncio
import time
...
async def get_candles_reliable(client, asset="AUDJPY_otc", period=60, retries=3):
    ...
    # 2) Resolver símbolo disponible (fallback OTC)
    asset_name, asset_data = await client.get_available_asset(asset, force_open=True)
    if not asset_data or not asset_data[2]:
        return []

    # 3) Reintentar consulta histórica con espera acotada
...
### Troubleshooting de Velas
- Usar `await client.connect()` y confirmar éxito antes de pedir velas.
- Para instrumentos OTC, usar formato `*_otc` (ejemplo: `AUDJPY_otc`).
- Si line/tick responde pero velas no, reintentar la consulta y mantener timeout para evitar esperas infinitas.
- Verificar que el activo esté abierto con `get_available_asset(..., force_open=True)` antes de pedir velas de mercado.
...
## Escuchar Dos Instrumentos en Paralelo (Mismo Proceso)
...
async def watch_symbol(email, password, symbol, period=60):
    ...
    if not ok:
        print(symbol, "error de conexión:", reason)
        return

    asset_name, asset_data = await client.get_available_asset(symbol, force_open=True)
    if not asset_data or not asset_data[2]:
        print(symbol, "no disponible")
        await client.close()
        return

    candles = await client.get_candles(asset_name, time.time(), 3600, period)
    print(symbol, "candles recibidos:", len(candles))
    await client.close()


async def main():
    await asyncio.gather(
        watch_symbol("user1@email.com", "pass1", "AUDJPY_otc"),
        watch_symbol("user2@email.com", "pass2", "EURUSD_otc"),
    )
...
Reglas recomendadas:
- Una instancia de `Quotex` por sesión.
- No compartir el mismo cliente entre logins concurrentes.
- Cerrar cada cliente cuando termine (`await client.close()`).