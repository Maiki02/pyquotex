...
### Fallback automático para token de sessão expirado

Desde abril de 2026, `connect()` inclui fallback automático para sessão inválida:

1. Se `session.json` tiver token, tenta conectar via WebSocket com esse token.
2. Se houver rejeição de autorização, a sessão em memória é limpa.
3. A autenticação HTTP é executada novamente com `email/password`.
4. A conexão WebSocket é tentada novamente com o token renovado.

Isso evita falhas definitivas por token expirado e melhora estabilidade em processos longos.
