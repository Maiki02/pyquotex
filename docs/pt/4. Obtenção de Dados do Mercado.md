...
### Comportamento interno de get_candles (atualizado)

- `get_candles()` usa trigger mínimo de assinatura (`subscribe_realtime_candle`) para forçar entrega do histórico quando o broker exige.
- No modo snapshot (`progressive=False`) a assinatura é sempre encerrada em `finally` com `unsubscribe_realtime_candle`.
- No modo progressivo (`progressive=True`) os canais adicionais de tempo real permanecem ativos.

Esse desenho evita timeout por falta de trigger e evita vazamento de rede em consultas históricas pontuais.

### Fluxo Confiável de Candles (Recomendado)
```python
import asyncio
import time
...
async def get_candles_reliable(client, asset="AUDJPY_otc", period=60, retries=3):
    ...
    # 2) Resolver símbolo disponível (fallback OTC)
    asset_name, asset_data = await client.get_available_asset(asset, force_open=True)
    if not asset_data or not asset_data[2]:
        return []

    # 3) Repetir requisição histórica com espera limitada
...
### Troubleshooting de Candles
- Use `await client.connect()` e confirme sucesso antes de solicitar candles.
- Para instrumentos OTC, use o formato `*_otc` (exemplo: `AUDJPY_otc`).
- Se line/tick funcionar e candles não, repita a requisição e mantenha timeout para evitar espera infinita.
- Verifique se o ativo está aberto com `get_available_asset(..., force_open=True)` antes de solicitar candles de mercado.
